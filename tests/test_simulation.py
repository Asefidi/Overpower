...
class OverpowerSimulationTests(unittest.TestCase):
    ...
    def test_baseline_stability(self) -> None:
        ...
        self.assertLess(latest.metrics["global_shortage_ratio"], 0.20)
        self.assertGreater(latest.readiness_index, 75.0)
    ...
    def test_hormuz_squeeze_raises_stress(self) -> None:
        ...
        self.assertGreaterEqual(stressed.metrics["global_shortage_ratio"] - baseline.metrics["global_shortage_ratio"], 0.03)
        self.assertGreaterEqual(baseline.readiness_index - stressed.readiness_index, 5.0)
        self.assertGreater(stressed.crude_price_by_locality["EUCOM"], baseline.crude_price_by_locality["EUCOM"])
        self.assertGreater(stressed.crude_price_by_locality["IRAN"], baseline.crude_price_by_locality["IRAN"])
        self.assertGreater(stressed.locality_shortage_ratio["CHINA"], baseline.locality_shortage_ratio["CHINA"])
    ...
    def test_world_scaffold_matches_scope(self) -> None:
        ...
        self.assertEqual(len(world.demand_agents), 82)
    ...
    def test_northcom_has_dedicated_military_buyer(self) -> None:
        world = build_world()
        military_buyers = [agent for agent in world.demand_agents if agent.agent_kind == "military"]
        self.assertEqual(len(military_buyers), 1)
        buyer = military_buyers[0]
        self.assertEqual(buyer.id, "northcom-military-buyer")
        self.assertEqual(buyer.locality, "NORTHCOM")
        ...
        self.assertGreater(buyer.price_priority["diesel"], 1.0)
        self.assertGreater(buyer.price_priority["jet"], 1.0)
        for product in PRODUCTS:
            modeled_demand = sum(
                agent.base_demand_bbl_week[product]
                for agent in world.demand_agents
                if agent.locality == "NORTHCOM"
            )
            self.assertAlmostEqual(modeled_demand, world.localities["NORTHCOM"].base_product_demand_bbl_week[product])
    ...
    def test_iran_baseline_is_not_perma_short(self) -> None:
        _, baseline = self._run("baseline")
        self.assertLess(baseline.locality_shortage_ratio["IRAN"], 0.05)
        self.assertGreaterEqual(baseline.product_fulfillment_ratio["IRAN"]["gasoline"], 0.95)
